...
int main(void)
{
	...
	string input_format = ifmt_ctx->iformat->name;
	...
	avformat_alloc_output_context2(&ofmt_ctx, NULL, output_format, outputfile);
			...
			if (pkt.data != NULL) {
				if (avcodec_decode_audio4(pACtx, pAFrame, &bGotSound, &pkt) >= 0) {
					if (bGotSound) {
						...
						if (bGotSound) {
							cout << "Write Frame [pts]=>" << pkt.pts << " [size]=>" << pkt.size << endl;
