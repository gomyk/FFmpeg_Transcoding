...
int main_wldnj()
{
	...
	if (!strcmp(output_type, "h264")) {
		video_encode_example("wild.mp4", AV_CODEC_ID_H264);
	}
...
const char *szFilePath = "wild.mp4";
...
ofmt = av_guess_format(NULL, "output.mp4", NULL);
ofcx = avformat_alloc_context();
ofcx->oformat = ofmt;
avio_open2(&ofcx->pb, "output.mp4", AVIO_FLAG_WRITE, NULL, NULL);
enc = avcodec_find_encoder(dec_ctx->codec->id);
enc_ctx = avcodec_alloc_context3(enc);
avcodec_get_context_defaults3(enc_ctx, enc);
//ost = avformat_new_stream( ofcx, (AVCodec *) iccx->codec );
ost = avformat_new_stream(ofcx, NULL);
avcodec_copy_context(ost->codec, dec_ctx);
...
snprintf(ofcx->filename, sizeof(ofcx->filename), "%s", "output.mp4");

//start reading packets from stream and write them to file

av_dump_format(fmt_ctx, 0, fmt_ctx->filename, 0);
av_dump_format(ofcx, 0, ofcx->filename, 1);

//timestart = timenow = get_time();
enc_ctx->bit_rate = 512 * 1024 * 2;
enc_ctx->bit_rate_tolerance = 512 * 1024 * 2;
enc_ctx->width = 320;
enc_ctx->height = 240;
...
outpkt.size = 0;
int ret;
ret  =avcodec_encode_video2(enc_ctx, &outpkt, frm, &got_output);
if (ret != 0) {
std::cout << "안되...." << std::endl;
}
if (got_output) {
